...
function AuditLogs() {
  ...
  const changedRows = (log) => {
    ...
    if (log.action === 'Created') {
      return Object.keys(newValues).map((k) => [k, null, newValues[k]]);
    }

    if (log.action === 'Deleted') {
      return Object.keys(oldValues).map((k) => [k, oldValues[k], null]);
    }
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <p style={{ color: '#64748b' }}>
        Every important action is recorded automatically — who did what, when, from where, and what changed.
      </p>
      ...
      >
        <div>
          ...
          <input
            placeholder="User, module, record..."
        ...
        <div>
          ...
          <select value={filters.user} onChange={setFilter('user')} style={inputStyle}>
        ...
        <div>
          ...
          <select value={filters.module} onChange={setFilter('module')} style={inputStyle}>
        ...
        <div>
          ...
          <select value={filters.action} onChange={setFilter('action')} style={inputStyle}>
        ...
        <div>
          ...
          <input type="date" value={filters.from} onChange={setFilter('from')} style={inputStyle} />
        </div>
        <div>
          <label style={labelStyle}>To</label>
          <input type="date" value={filters.to} onChange={setFilter('to')} style={inputStyle} />
        </div>
      </div>
            ...
            {logs.map((log) => (
              <Fragment key={log.id}>
                <tr style={{ borderBottom: '1px solid #e2e8f0' }}>
                  ...
                  <td style={{ padding: '10px' }}>
                    #{log.record_id} {log.record_label || ''}
                  </td>
                  <td style={{ padding: '10px' }}>
                    <span
                      style={{
                ...
                {expanded === log.id && (
                  <tr style={{ background: '#f8fafc' }}>
                    <td colSpan="9" style={{ padding: '15px' }}>
                      ...
                      {changedRows(log).length === 0 && (
                        <div style={{ color: '#64748b' }}>No field details recorded.</div>
                      )}
                      <div
                        style={{
                          display: 'grid',
                      ...
                      >
                        ...
                        {changedRows(log).map(([fieldName, oldValue, newValue]) => (
                          <Fragment key={fieldName}>
                            <div style={{ padding: '4px 0' }}>{fieldName}</div>
                            <div style={{ padding: '4px 0', wordBreak: 'break-all' }}>
            ...
            {logs.length === 0 && (
              <tr>
                <td colSpan="9" style={{ padding: '20px', textAlign: 'center', color: '#64748b' }}>
                  No audit logs found. Perform some actions (create/edit/delete) and refresh.
                </td>
              </tr>
            )}
          </tbody>
      ...
      >
        <span style={{ color: '#64748b', fontSize: '14px' }}>
          Showing {total === 0 ? 0 : from}–{to} of {total} logs
        </span>
