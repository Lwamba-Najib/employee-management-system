...
const emptyForm = {
  employee_id: '', payroll_month: '', payroll_year: '2026',
  basic_salary: '0', housing_allowance: '0', transport_allowance: '0', medical_allowance: '0',
  other_allowances: '0', bonus: '0', paye: '0', nssf_deduction: '0', loan_deduction: '0',
  other_deductions: '0', payment_date: '', payment_status: 'Pending',
};
...
function SalaryManager() {
  ...
  const gross = num(form.basic_salary) + num(form.housing_allowance) + num(form.transport_allowance) + num(form.medical_allowance) + num(form.other_allowances) + num(form.bonus);
  const totalDeductions = num(form.paye) + num(form.nssf_deduction) + num(form.loan_deduction) + num(form.other_deductions);
  ...
  useEffect(() => { load(1); }, [search, fMonth, fYear, fStatus]);
  ...
  const openEdit = (s) => {
    ...
    Object.keys(next).forEach((k) => { next[k] = s[k] != null ? String(s[k]) : ''; });
    setForm(next);
    setShowModal(true);
  };
  ...
  const handleDelete = async (s) => {
    if (!window.confirm('Delete salary record for ' + (s.employee_name || 'employee') + ' (' + s.payroll_month + ' ' + s.payroll_year + ')?')) return;
    try {
      const res = await fetch(API + '/salaries/' + s.id, {
        method: 'DELETE',
        headers: { Authorization: 'Bearer ' + token, Accept: 'application/json' },
      });
      if (res.ok) { alert('Record deleted.'); load(page); }
      else alert('Delete failed (HTTP ' + res.status + ')');
    } catch (e) { alert('Error connecting to server.'); }
  };

  const markPaid = async (s) => {
    if (!window.confirm('Mark ' + (s.employee_name || 'record') + ' — ' + s.payroll_month + ' ' + s.payroll_year + ' as PAID?')) return;
    try {
      const res = await fetch(API + '/salaries/' + s.id, {
        method: 'PUT',
        headers: {
        ...
        body: JSON.stringify({ payment_status: 'Paid', payment_date: new Date().toISOString().slice(0, 10) }),
      });
      if (res.ok) { alert('Marked as paid. 💸'); load(page); }
    ...
    <div style={{ padding: '20px' }}>
      ...
      {stats && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '15px', margin: '20px 0' }}>
          <div style={{ background: '#fff', padding: '20px', borderRadius: '8px', textAlign: 'center' }}><div style={{ fontSize: '12px', color: '#64748b' }}>TOTAL RECORDS</div><div style={{ fontSize: '28px', fontWeight: '700' }}>{stats.total_records}</div></div>
          <div style={{ background: '#fff', padding: '20px', borderRadius: '8px', textAlign: 'center', border: '1px solid #f59e0b' }}><div style={{ fontSize: '12px', color: '#64748b' }}>PENDING</div><div style={{ fontSize: '28px', fontWeight: '700', color: '#d97706' }}>{stats.pending}</div></div>
          <div style={{ background: '#fff', padding: '20px', borderRadius: '8px', textAlign: 'center', border: '1px solid #22c55e' }}><div style={{ fontSize: '12px', color: '#64748b' }}>PAID</div><div style={{ fontSize: '28px', fontWeight: '700', color: '#16a34a' }}>{stats.paid}</div></div>
          <div style={{ background: '#fff', padding: '20px', borderRadius: '8px', textAlign: 'center', border: '1px solid #ea580c' }}><div style={{ fontSize: '12px', color: '#64748b' }}>TOTAL NET</div><div style={{ fontSize: '28px', fontWeight: '700', color: '#ea580c' }}>{money(stats.total_net)}</div></div>
        </div>
      )}

      <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', margin: '15px 0' }}>
        <input placeholder="Search employee name / #..." value={search} onChange={(e) => setSearch(e.target.value)} style={{ ...inputStyle, width: '220px' }} />
        <select value={fMonth} onChange={(e) => setFMonth(e.target.value)} style={{ ...inputStyle, width: '150px' }}>
          <option value="">All months</option>
          {MONTHS.map((m) => <option key={m}>{m}</option>)}
        </select>
        <input placeholder="Year (e.g. 2026)" value={fYear} onChange={(e) => setFYear(e.target.value)} style={{ ...inputStyle, width: '130px' }} />
          ...
          <tbody>
            {salaries.map((s) => (
              <tr key={s.id} style={{ borderBottom: '1px solid #e2e8f0' }}>
                ...
                <td style={{ padding: '10px' }}>
                  <span style={{ background: s.payment_status === 'Paid' ? '#f0fdf4' : '#fffbeb', color: s.payment_status === 'Paid' ? '#16a34a' : '#d97706', padding: '4px 8px', borderRadius: '4px', fontWeight: '600' }}>{s.payment_status}</span>
                </td>
                <td style={{ padding: '10px' }}>
                  {s.payment_status === 'Pending' && (
                    <button onClick={() => markPaid(s)} style={{ marginRight: '8px', padding: '4px 8px', color: '#16a34a', background: '#f0fdf4', border: '1px solid #86efac', borderRadius: '4px', cursor: 'pointer' }}>Pay</button>
                  )}
                  <button onClick={() => openEdit(s)} style={{ marginRight: '8px', padding: '4px 8px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '4px', cursor: 'pointer' }}>Edit</button>
                  <button onClick={() => handleDelete(s)} style={{ padding: '4px 8px', color: '#dc2626', background: '#fef2f2', border: '1px solid #fca5a5', borderRadius: '4px', cursor: 'pointer' }}>Delete</button>
                </td>
              </tr>
            ))}
            {salaries.length === 0 && (
              <tr><td colSpan="8" style={{ padding: '20px', textAlign: 'center', color: '#64748b' }}>No salary records found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '15px' }}>
        <span style={{ color: '#64748b', fontSize: '14px' }}>Showing {total === 0 ? 0 : from}–{to} of {total} records</span>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
                <div>
                  ...
                  <select name="employee_id" value={form.employee_id} onChange={handleChange} required style={inputStyle}>
                    <option value="">Select employee...</option>
                    {employees.map((emp) => (
                      <option key={emp.id} value={emp.id}>{emp.employee_number ? emp.employee_number + ' — ' : ''}{emp.name}</option>
                    ))}
                  </select>
                </div>
                <div>
                  <label style={labelStyle}>Payroll Month *</label>
                  <select name="payroll_month" value={form.payroll_month} onChange={handleChange} required style={inputStyle}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginTop: '15px', background: '#f8fafc', padding: '15px', borderRadius: '8px' }}>
                <div><div style={labelStyle}>GROSS SALARY</div><div style={{ fontSize: '20px', fontWeight: '700', color: '#16a34a' }}>{money(gross)}</div></div>
                <div><div style={labelStyle}>TOTAL DEDUCTIONS</div><div style={{ fontSize: '20px', fontWeight: '700', color: '#dc2626' }}>{money(totalDeductions)}</div></div>
                <div><div style={labelStyle}>NET SALARY</div><div style={{ fontSize: '20px', fontWeight: '700', color: '#2563eb' }}>{money(net)}</div></div>
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '20px' }}>
                ...
                <button type="submit" style={btn('#2563eb')}>{editingId ? 'Save Changes' : 'Create Record'}</button>
              </div>
            </form>
          </div>
        </div>
